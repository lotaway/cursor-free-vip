...
class Translator:
    def __init__(self):
        ...
        if (
            self.config
            and self.config.has_section("Language")
            and self.config.has_option("Language", "fallback_language")
        ):
        ...
        if (
            self.config
            and self.config.has_section("Language")
            and self.config.has_option("Language", "current_language")
        ):
            ...
            if saved_language and saved_language.strip():
                ...
                if self.config.has_section("Language"):
                    ...
                    config_dir = os.path.join(
                        get_user_documents_path(), ".cursor-free-vip"
                    )
    ...
    def download_language_file(self, lang_code):
        ...
        print(
            f"{Fore.YELLOW}{EMOJI['INFO']} Languages are now integrated into the package, no need to download.{Style.RESET_ALL}"
        )
        return False
    ...
    try:
        ...
        if choice.isdigit() and 0 <= int(choice) < languages_count:
            ...
            if config and config.has_section("Language"):
                ...
                print(
                    f"{Fore.GREEN}{EMOJI['SUCCESS']} {translator.get('menu.language_config_saved', language=translator.get(f'languages.{selected_language}', fallback=selected_language))}{Style.RESET_ALL}"
                )
            ...
            print(
                f"{Fore.RED}{EMOJI['ERROR']} {translator.get('menu.lang_invalid_choice', lang_choices=f'0-{languages_count - 1}')}{Style.RESET_ALL}"
            )
            return False
...
def check_latest_version():
    ...
    try:
        ...
        try:
            ...
            if (
                github_response.status_code == 403
                and "rate limit exceeded" in github_response.text.lower()
            ):
                print(
                    f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.rate_limit_exceeded', fallback='GitHub API rate limit exceeded. Trying backup API...')}{Style.RESET_ALL}"
                )
                raise Exception("Rate limit exceeded")

            # Check if repository/releases not found
            if github_response.status_code == 404:
                print(
                    f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.no_releases', fallback='No releases found in repository, skipping update check')}{Style.RESET_ALL}"
                )
                return
            ...
            github_error = str(e)
            print(
                f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.github_api_failed', fallback='GitHub API failed, trying backup API...')}{Style.RESET_ALL}"
            )

            # If GitHub API fails, try backup API (but skip if GitHub returned 404)
            if github_response and github_response.status_code != 404:
                try:
                    backup_headers = {
                    ...
                    backup_response = requests.get(
                        "https://pinnumber.rr.nu/badges/release/lotaway/cursor-free-vip",
                    ...
                    if backup_response.status_code != 200:
                        raise Exception(
                            f"Backup API returned status code {backup_response.status_code}"
                        )
                    ...
                    # If both APIs fail, show warning and continue
                    print(
                        f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.both_apis_failed', fallback='Both update APIs failed, continuing with current version')}{Style.RESET_ALL}"
                    )
                    return
            else:
                # GitHub returned 404, skip backup API
                return
        ...
        if isinstance(current_version_tuple, tuple) and isinstance(
        ...
        if is_newer_version_available:
            ...
            try:
                changelog_url = "https://raw.githubusercontent.com/lotaway/cursor-free-vip/main/CHANGELOG.md"
                ...
                if changelog_response.status_code == 200:
                    ...
                    if latest_changes:
                        ...
                        print(
                            f"{Fore.CYAN}{translator.get('updater.changelog_title')}:{Style.RESET_ALL}"
                        )
                        ...
                        for i, line in enumerate(
                            changes_lines[1:11]
                        ):  # skip version number line, max 10 lines
                            if line.strip():
            ...
            while True:
                choice = input(
                    f"\n{EMOJI['ARROW']} {Fore.CYAN}{translator.get('updater.update_confirm', choices='Y/n')}: {Style.RESET_ALL}"
                ).lower()
                if choice in ["", "y", "yes"]:
                    break
                elif choice in ["n", "no"]:
            ...
            try:
                ...
                if platform.system() == "Windows":
                    update_command = "irm https://raw.githubusercontent.com/lotaway/cursor-free-vip/main/scripts/install.ps1 | iex"
                    subprocess.run(
                        [
                            "powershell",
                            "-NoProfile",
                    ...
                    install_script_url = "https://raw.githubusercontent.com/lotaway/cursor-free-vip/main/scripts/install.sh"
                ...
                print(
                    f"{Fore.RED}{EMOJI['ERROR']} {translator.get('updater.update_failed', error=str(update_error))}{Style.RESET_ALL}"
                )
                print(
                    f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.manual_update_required')}{Style.RESET_ALL}"
                )
                return
        else:
            ...
            if current_version_tuple > latest_version_tuple:
                print(
                    f"{Fore.GREEN}{EMOJI['SUCCESS']} {translator.get('updater.development_version', current=version, latest=latest_version)}{Style.RESET_ALL}"
                )
            else:
                print(
                    f"{Fore.GREEN}{EMOJI['SUCCESS']} {translator.get('updater.up_to_date')}{Style.RESET_ALL}"
                )

    except requests.exceptions.RequestException as e:
        print(
            f"{Fore.RED}{EMOJI['ERROR']} {translator.get('updater.network_error', error=str(e))}{Style.RESET_ALL}"
        )
        print(
            f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.continue_anyway')}{Style.RESET_ALL}"
        )
        return

    except Exception as e:
        print(
            f"{Fore.RED}{EMOJI['ERROR']} {translator.get('updater.check_failed', error=str(e))}{Style.RESET_ALL}"
        )
        print(
            f"{Fore.YELLOW}{EMOJI['INFO']} {translator.get('updater.continue_anyway')}{Style.RESET_ALL}"
        )
