...
try:
    ...
    print(
        f"{Fore.YELLOW}googletrans library not found. Will use web scraping fallback method.{Style.RESET_ALL}"
    )
    print(
        f"{Fore.YELLOW}To install googletrans: pip install googletrans==4.0.2{Style.RESET_ALL}"
    )
    import requests
...
def translate(text, source, target):
    ...
    print(
        f"{Fore.CYAN}Translating from {source_lang} to {target_lang}{Style.RESET_ALL}"
    )
...
def translate_web_scraping(text, source, target):
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                Fore.RED
                + f"Request failed with status code {response.status_code} for: {text}"
            )
            return text
    except Exception as e:
...
def process_language(en_filename, other_filename, create_backup=None):
    ...
    print(
        f"{Fore.CYAN}Processing: {other_filename} (Translating to {get_google_lang_code(other_lang)}){Style.RESET_ALL}"
    )
    ...
    try:
        ...
        print(
            f"{Fore.YELLOW}Target file not found. Creating a new file.{Style.RESET_ALL}"
        )
        other = {}
    except json.JSONDecodeError:
        # If target file is invalid JSON, create an empty one
        print(
            f"{Fore.YELLOW}Target file contains invalid JSON. Creating a new file.{Style.RESET_ALL}"
        )
        other = {}
    ...
    if not missing:
        print(
            f"{Fore.GREEN}No missing keys found. Translation is complete!{Style.RESET_ALL}"
        )
        return True
    ...
    with ThreadPoolExecutor(
        max_workers=3
    ) as executor:  # Further reduced workers for googletrans 4.0.2
        future_to_key = {
        ...
        for future in as_completed(future_to_key):
            ...
            try:
                ...
                completed += 1
                print(
                    f"{Fore.CYAN}[{completed}/{total}] Translated [{key}]: '{value}' -> "
                    + Fore.MAGENTA
                    + f"'{translated}'"
                )
    ...
    if create_backup is None and os.path.exists(other_path):
        while True:
            backup_choice = input(
                f"{Fore.CYAN}Create backup file? (y/N): {Style.RESET_ALL}"
            ).lower()
            if backup_choice in ["y", "yes"]:
                ...
                print(
                    f"{Fore.RED}Invalid choice. Please enter 'y' or 'n'.{Style.RESET_ALL}"
                )
...
def main():
    ...
    if "en.json" not in json_files:
        print(
            f"{Fore.RED}Error: 'en.json' not found in locales directory. This file is required as the source for translations.{Style.RESET_ALL}"
        )
        return False
    ...
    available_languages = [
        lang for lang in available_languages if lang not in existing_lang_codes
    ]

    # Display menu
    print(f"\n{Fore.CYAN}{'=' * 50}{Style.RESET_ALL}")
    print(
        f"{Fore.CYAN}Translation Tool - Select target language to update{Style.RESET_ALL}"
    )
    print(f"{Fore.CYAN}{'=' * 50}{Style.RESET_ALL}")
    ...
    while True:
        try:
            ...
            if choice < 0 or choice > max_choice:
                print(
                    f"{Fore.RED}Invalid choice. Please enter a number between 0 and {max_choice}.{Style.RESET_ALL}"
                )
                continue
    ...
    if choice == 0:
        while True:
            backup_choice = input(
                f"{Fore.CYAN}Create backup files? (y/N): {Style.RESET_ALL}"
            ).lower()
            if backup_choice in ["y", "yes"]:
                create_backup = True
                break
    ...
    if choice == 0:
        ...
        for target_file in target_files:
            try:
                ...
                print(
                    f"{Fore.RED}Error processing {target_file}: {str(e)}{Style.RESET_ALL}"
                )
        ...
        while True:
            try:
                ...
                if lang_choice < 1 or lang_choice > len(available_languages):
                    print(
                        f"{Fore.RED}Invalid choice. Please enter a number between 1 and {len(available_languages)}.{Style.RESET_ALL}"
                    )
                    continue
                ...
                if new_file in json_files:
                    print(
                        f"{Fore.YELLOW}Warning: {new_file} already exists. It will be updated with missing translations.{Style.RESET_ALL}"
                    )
